...
## How to run: 
Running DatasetGAN-SG2 is similar to running the original [DatasetGAN](https://github.com/nv-tlabs/datasetGAN_release) implementation. 
All required commands are also present in the  `run_DatasetGAN_SG2.ipynb` Jupyter Notebook.
The main difference are some changes in data directory folders (input and output), however, these can easily be changed by manipulating the paths in the scripts. 

### Create training examples
To create training examples simply annotate the desired number of the images generated by `make_training_data.py` with the desired number of regions. For this you can use [GIMP](https://www.gimp.org/) or any other software. Save the annotations and original images to the `eyes_GIMP` directory as seen in the example. 
Next, preprocess the annotations so that the classes are in the range `{0, 1, 2, ... num_classes}` and that the annotations are saved as `.npy` files. For this, you can also use the `preprocess_annotated_images_and_latents.ipynb` Jupyter Notebook, however, make sure that the values correspond to your annotations.
...
| Path | Description
| :--- | :----------
| Experiments | Main experiments directory
...
| &ensp;&ensp;&boxvr;&nbsp; Experiment_1 | ...
| &ensp;&ensp;&ensp;&ensp;&boxvr;&nbsp; ... | ...

This DatasetGAN version was implemented during the development of the [BiOcularGAN](https://github.com/dariant/BiOcularGAN) framework, as a proof of concept. The final version uses a Dual-branch StyleGAN2 approach to generate photorealistic bimodal images. To cite this work please use the below information.
